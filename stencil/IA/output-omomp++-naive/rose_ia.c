...
#pragma omp single 
{
    ...
    __parreg_arglist = ((void **)(malloc(5 * sizeof(void *))));
    __parreg_arglist[1] = &iters;
...
#pragma omp for  private(i)
  for (i = 1; i <= N; i++) {
    if (arg_list_parallel_for_body1_0[i] == NULL) {
      arg_list_parallel_for_body1_0[i] = malloc(5 * sizeof(void *));
    }
...
//    }
    ...
    if (arg_list_sum_0[i] == NULL) {
      arg_list_sum_0[i] = malloc(5 * sizeof(void *));
    }
    arg_list_sum_0[i][0] = diff;
    arg_list_sum_0[i][1] = &delta;
    sum_cps(arg_list_sum_0[i],parallel_for_body1__while__0_cps_cont_env4,phase_num);
  }
   else {
    struct __cont_env *parallel_for_body1__while__0_cps_cont_env4;
//    if (old_env != NULL) {
