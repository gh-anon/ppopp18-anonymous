...
#pragma omp single 
	{
		...
		uwomp_worklist_init((omp_get_num_threads()), N, 1, 32766);
...
void sum_cps(void **arg_list,struct __cont_env *contn_env,int phase_num,struct __cont_env *old_env)
{
  ...
  if (contn_env != NULL) {
	...
	if (contn_env == NULL) {
		//TODO: Not sure the following check is needed here because
		//it will not ever be called??
		//
		...
		//			}
		// FIXME: As part of static optimization we need the task id i
		// for the following call. But it is not available here. So commenting
		// the following.
		//uwomp_execute_ready_worklist((omp_get_thread_num()));
	}
}
...
void parallel_for_body1__while__0_cps_cont4(void **arg_list,struct __cont_env *contn_env,int phase_num,struct __cont_env *old_env)
{
  ...
  //uwomp_barrier(old_env,phase_num);
	//TODO:
	// i is taken from arglist which is only needed for static optimization.
  int i = (int )arg_list[0];
  uwomp_barrier(old_env, i, phase_num);
}
