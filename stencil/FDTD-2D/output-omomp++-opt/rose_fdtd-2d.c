...
#pragma omp single 
{
	...
	if (ready_worklist == NULL) {
        ...
        uwomp_worklist_init((omp_get_num_threads()), nx, 0, 32765);
	}
				...
				if (OmpRegBody1_cps_cps_cont_env_1_1 == NULL) {
					OmpRegBody1_cps_cps_cont_env_1_1 = calloc(nx, sizeof(struct __cont_env *));
				}
      }
      void **__parreg_arglist;
      __parreg_arglist = ((void **)(malloc(2 * sizeof(void *))));
      __parreg_arglist[0] = &nx;
      __parreg_arglist[1] = &ny;
...
void parallel_for_body1_cps(void **arg_list,struct __cont_env *contn_env,int phase_num,struct __cont_env *old_env)
{
  ...
  //int *nx = (int *)arg_list[1];
  int *ny = (int *)arg_list[2];
	int vny = *ny;
	int j;
  for (j = 1; j <  vny; j++) 
...
void OmpRegBody1_cps(void **arg_list,struct __cont_env *contn_env,int phase_num,struct __cont_env *old_env)
{
  int *nx = (int *)arg_list[0];
  int *ny = (int *)arg_list[1];
  int i;
  int j;
...
#pragma omp for  private(i,j)
  for (i = 0; i <  *nx; i++) {
    ...
    arg_list_parallel_for_body1_0[i][1] = nx;
    arg_list_parallel_for_body1_0[i][2] = ny;
