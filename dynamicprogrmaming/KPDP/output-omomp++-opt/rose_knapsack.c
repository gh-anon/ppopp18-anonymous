...
#pragma omp single 
{
    ...
    //arg_list_knapSack_0 = calloc(C_plus - 0,sizeof(void **));
      ...
      uwomp_worklist_init((omp_get_num_threads()), C_plus - 0, 0, 32764);
    }
...
#pragma omp for 
  for (k = 0; k < C_plus; k++) {
    ...
    uwomp_add_to_local_pending_worklist(OmpRegBody1_cps_cps_cont_env_1_1, k, 0);
  }
...
//0 to N-1 row index
  if (i >= N) {
    ...
    if (contn_env == NULL) {
			...
			if((k + 1) <= max_task_id_array[omp_get_thread_num()])
			{ 
				int local_index = get_local_index(k + 1);
  ...
  uwomp_barrier(old_env, k, phase_num);
}
